...
SymbolTableEntry* Symbol_create(SymbolType type, const char * name , unsigned int line, unsigned int scope, int isactive)
{
...
void SymbolTable_print()
{
	...
	for (i = 0; i <= max_scope; i++)
	{
		...
		printf("\nIn scope[%d]:\n\t|%-25s|%-12s|%-12s|%-12s", i, "ID", "Type", "Line", "isActive");
		while (tmphead != NULL)
		{
			
			printf("\n\t|%-25s|%-12s|%-12d|%-12d\t", tmphead->name, SymbolTableTypeArray[tmphead->type], tmphead->line, tmphead->isActive);
...
char* createNamelessFunc()
{
	...
	strcpy(buffer, "_f");
	sprintf(buffer2, "%d", namelesscount);
