...
char* OpcodeArray[] = {"=", "addition \'+\'", "subtraction \'-\'", "multiplication \'*\'",
							 "divition \'/\'", "modulo \'%\'"};


/**
 * @brief Expand the quads array.
 */
...
/**
 * @param		op			Opcode of the produced expression
 * @param		arg1		First argument of the produced expression.
 * @param		arg2		Second argument of the produced expression.
 * @param		result		Result of the produced expression.
 * @param		label		The label of the produced expression.
 * @param		line		The line of occurence of the produced expression.
 * @brief					Creates a new quad and insert it in the table
 */
...
void emit_param(expr* e) {
	...
	emit_param(e->next);
	emit(param, (expr*)NULL, (expr*)NULL, e, -1, -1);
}


/**
 * @param		e		The expression to be checked.
 * @brief				Emits the expression only in the case the expression being parsed is of tableitem_e type.
 */
expr* emit_iftableitem(expr* e) {
	if (e->type != tableitem_e) {
		...
		result->sym = newtemp();
		result->sym->scopespace = currscopespace();
		result->sym->offset = currscopeoffset();
		SymbolTable_insert(result->sym);
		//incurrscopeoffset();
...
/**
 * @param 		type		The desired type for the new expression.
 * @brief 					Creates a new expresion with the said type
 */
...
/**
 * @param 		symbol		
 * @brief					Creates a new lvalue expression with 
 */
...
/**
 * @param	numConst	A constant number.
 * @brief				Creates and returns a new expression containing a number.
 */
...
expr* make_call(expr* lvalue, expr* elist) {
	...
	emit(call, (expr*)NULL, (expr*)NULL, func, -1, -1);
	expr* result = newexpr(var_e);
	result->sym = newtemp();
	result->sym->scopespace = currscopespace();
	result->sym->offset = currscopeoffset();
	...
	if (result->sym->scopespace == program_variable) {
		inc_globals();
	}

	emit(getretval, (expr*)NULL, (expr*)NULL, result, -1, -1);
	return result;
}
 ...
 * @param		base		
 * @param		front		
 * @brief					Adds front ahead of base and returns a pointer to new base.
 */
...
/**
 * @param		e			The expression to be checked.
 * @brief					Terminates parsing with exit code -1 in case an illegal expression has being cast to unary.
 */
...
SymbolTableEntry* newtemp(void) {
	...
	if (sym == NULL) {
		if (scope == 0) {
			return Symbol_create(GLOBAL, name, 0, scope, 1);
		}
		else {
...
/**
 * @param		quadNo		The processed quads number.
...
void print_quad_table() {
	...
	printf("---- QUAD TABLE ----\n");
	printf("%-3s %-20s%-12s%-12s%-12s%-12s%-12s\n", "#", "opcode", "arg1", "arg2", "result", "label", "taddress");
	while (i < current_quad) {
		...
		switch (quad_entry->op) {
			...
			case if_noteq:
				...
				break;
			case if_lesseq:
				printf("%-20s", "IF_LESSEQ");
				break;
			case if_greatereq:
				printf("%-20s", "IF_GREATEREQ");
				break;
			case if_less:
				printf("%-20s", "IF_LESS");
				break;
			case if_greater:
				printf("%-20s", "IF_GREATER");
				break;
			case jump:
				printf("%-20s", "JUMP");
				break;
			...
			case retur:
				...
				break;
			case getretval:
				printf("%-20s", "GETRETVAL");
				break;
			case funcstart:
				printf("%-20s", "FUNCSTART");
				break;
			case funcend:
				printf("%-20s", "FUNCEND");
				break;
			case tablecreate:
				printf("%-20s", "TABLECREATE");
				break;
			case tablegetelem:
				printf("%-20s", "TABLEGETELEM");
				break;
			case tablesetelem:
				printf("%-20s", "TABLESETELEM");
				break;
		}
		...
		if (quad_entry->arg2 != NULL) {
			switch (quad_entry->arg2->type) {
				case tableitem_e:
					printf("%-12s", quad_entry->arg2->index->strConst);
					break;
				case var_e:
				case programfunc_e:
				case libraryfunc_e:
		...
		printf("%-12d%-12d", quad_entry->label, quad_entry->taddress);
