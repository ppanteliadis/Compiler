...
objectdef: LEFT_BRACKET elist RIGHT_BRACKET
			{
				...
				t->sym = newtemp();
				emit(tablecreate, t, (expr*)NULL, (expr*)NULL, -1, yylineno);
				int i = 0;
				expr* x;
...
funcdef: funcprefix funcargs funcblockstart funcbody funcblockend
			{
				...
				$$ = $<exprNode>1->sym;
				emit(funcend, expr_lvalue($<exprNode>1->sym), (expr*)NULL, (expr*)NULL, -1, yylineno);
			}
		;
...
funcprefix: FUNCTION funcname
			{
				...
				if (sym == NULL) {
					...
					if (isLibraryFunction($2)==1) {
						...
						emit(funcstart, expr_lvalue($<exprNode>$->sym), NULL, NULL, -1, yylineno);
...
returnstmt: RETURN {returnflag = 1;} SEMICOLON
			{
				fprintf(yyout,"return ;\n");

				emit(retur, (expr*)NULL, (expr*)NULL, (expr*)NULL, -1, yylineno);
			}
		| RETURN {returnflag = 1;} expr SEMICOLON
			{
				fprintf(yyout,"return expr;\n");

				emit(retur, $<exprNode>3, (expr*)NULL, (expr*)NULL, -1, yylineno);
			}
		;
