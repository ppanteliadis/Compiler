...
unsigned userfuncs_newfunc(SymbolTableEntry *sym){
    ...
    for(i=0; i<currUSERFUNC; i++){
        if(( strcmp(t_userfunc[i].name,sym->value.funcVal->name) == 0)&&(sym->value.funcVal->taddress == t_userfunc[i].taddress)){
            return i;
        }
    }
    ...
    function->name = (char * )malloc(sizeof(char)*(strlen(sym->value.funcVal->name)+1));
    function->name[0]='\0';
    strcpy(function->name , sym->value.funcVal->name);
    function->name[strlen(sym->value.funcVal->name)]='\0';

    function->taddress = sym->value.funcVal->taddress;
    function->totallocals = sym->value.funcVal->totallocals;
